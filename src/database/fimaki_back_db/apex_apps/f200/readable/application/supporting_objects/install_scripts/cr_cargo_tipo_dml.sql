begin
  ...
  using (select 'CARGO' as codigo,
                'CARGO' as nombre,
                'CARGO' as descripcion,
                'S' as sistema_sn,
                'S' as activo_sn
         from dual) y
  ...
  -- === CARGO (referencias con alias) ===
  merge into codigo_referencia x
  using (
    select 'CARGO' as codigo_ref_tipo, 'ANALISTA' as codigo, 'Analista' as nombre, 'nan' as descripcion, 10 as secuencia, 'S' as activo_sn from dual
    union all
    select 'CARGO' as codigo_ref_tipo, 'GGRAL' as codigo, 'Gerente General' as nombre, 'Gerente General' as descripcion, 20 as secuencia, 'S' as activo_sn from dual
    union all
    select 'CARGO' as codigo_ref_tipo, 'CONTADOR' as codigo, 'Contador General' as nombre, '.' as descripcion, 30 as secuencia, 'S' as activo_sn from dual
  ) y
