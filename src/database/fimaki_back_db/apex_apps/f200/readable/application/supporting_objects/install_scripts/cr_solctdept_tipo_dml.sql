begin
  ...
  using (select 'SOLCTDEPT' as codigo,
                'SOLCTDEPT' as nombre,
                'SOLCTDEPT' as descripcion,
                'S' as sistema_sn,
                'S' as activo_sn
         from dual) y
  ...
  -- === SOLCTDEPT (referencias con alias) ===
  merge into codigo_referencia x
  using (
    select 'SOLCTDEPT' as codigo_ref_tipo, 'PENDIENTE' as codigo, 'Pendiente de validación' as nombre, 'Pendiente de validación' as descripcion, 10 as secuencia, 'S' as activo_sn from dual
  ) y
