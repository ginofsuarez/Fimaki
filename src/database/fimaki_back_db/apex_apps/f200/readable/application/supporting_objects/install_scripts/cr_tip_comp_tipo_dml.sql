begin
  ...
  using (select 'TIP_COMP' as codigo,
                'TIP_COMP' as nombre,
                'TIP_COMP' as descripcion,
                'S' as sistema_sn,
                'S' as activo_sn
         from dual) y
  ...
  -- === TIP_COMP (referencias con alias) ===
  merge into codigo_referencia x
  using (
    select 'TIP_COMP' as codigo_ref_tipo, 'FAC_ELCTR' as codigo, 'Factura electrónica' as nombre, '.' as descripcion, 10 as secuencia, 'S' as activo_sn from dual
  ) y
